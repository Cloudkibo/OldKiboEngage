  ...
  return (
    ...
    (selected ?
      <div className="list-group-item"
           style={{'width': '100%', 'backgroundColor': 'rgba(195, 188, 188, 0.4)', 'height': 'auto'}}
           onClick={props.onClickSession}>
        ...
        <span className='list-group-item-heading'
              style={customername}>{props.customer.user_id.first_name + ' ' + props.customer.user_id.last_name}</span>
        <span style={changefont}><i className="glyphicon glyphicon-calendar"
                                    style={{fontSize: 10}}/>{handleDate(props.customer.lastmessage.timestamp)}</span>

        <br/>
        <span><i className="fa fa-globe"/>{'Page: ' + props.customer.pageid.pageTitle}</span>
        <span style={changefont}><i className="glyphicon glyphicon-time"
                                    style={{fontSize: 10}}/>{handleTime(props.customer.lastmessage.timestamp)}</span>

        {(unreadCount == 0 ?
            <span className='badge' style={rightStyle}></span> :
        ...
        <br/>
        {
          agentname.length > 0 ?
            ...
            <span style={statusstyle}><i
              className="fa fa-headphones"/>{props.customer.status == 'assigned' ? props.customer.status + ' to ' : props.customer.status + ' by '} {agentname[agentname.length - 1]}</span>
            :
      ...
      unreadCount > 0 ?
        ...
        :
        (<div className="list-group-item" style={{'width': '100%', 'cursor': 'pointer', 'height': 'auto'}}
              onClick={props.onClickSession}>
            <img src={props.customer.user_id.profile_pic} width="36" height="36" className="user-avatar" style={hleft}/>
            <span className='list-group-item-heading'
                  style={customername}>{props.customer.user_id.first_name + ' ' + props.customer.user_id.last_name}</span>
